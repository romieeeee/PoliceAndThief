...
2. [환경변수](#2-환경변수)
    *   [2.1. Frontend (local.properties, keystore)](#21--frontend-localproperties-keystore)
        *   [2.1.1 keystore](#211-keystore)
...
5. [외부서비스](#5-외부서비스)
    ...
    *   [5.4 Google map](#54-google-map)
        *   [5.4.1. 프로젝트 생성](#541-프로젝트-생성)
        *   [5.4.2. 지도 생성 및 스타일 적용](#542-지도-생성-및-스타일-적용)
        *   [5.4.3. local.properties 작성](#543-localproperties-작성)
    *   [5.5. Naver Reverse Geocode](#55-naver-reverse-geocode)
        *   [5.5.1. Application 등록](#551-application-등록)
        *   [5.5.2. Application Key 확인](#552-application-key-확인)
        *   [5.5.3. local.properties 작성](#553-localproperties-작성)
    *   [5.6. FCM](#56-fcm)
        *   [5.6.1. 프로젝트 생성](#561-프로젝트-생성)
        *   [5.6.2. 안드로이드 앱 등록](#562-안드로이드-앱-등록)
        *   [5.6.3. google-services.json 배치](#563-google-servicesjson-배치)
---
...
### 2.1  Frontend (`local.properties`, `keystore`)
* `GOOGLE_MAP_API_KEY`, `INGAME_MAP_ID`, `SETTING_MAP_ID`, `CLIENT_ID`, `CLIENT_SECRET`
* `KAKAO_NATIVE_APP_KEY`
* `SIGNING_STORE_FILE`, `SIGNING_STORE_PASSWORD`, `SIGNING_KEY_ALIAS`, `SIGNING_KEY_PASSWORD`

#### 2.1.1 keystore
사용자 인증 정보를 안전하게 로컬에 저장하기 위해 DataStore를 이용합니다.

* **keystore 생성**: 프로젝트 루트 디렉토리에 `keystore` 디렉토리를 생성합니다.
* **keystore 파일 배치**: `team_debug.keystore` 파일을 방금 만든 `root/keystore`에 배치합니다.
...
### 5.4 Google map
경기 구역 설정과 위치를 표시하기 위해 스타일을 커스텀 한 **구글 맵**을 사용합니다. 

#### 5.4.1 프로젝트 생성
* **Google Cloud Console 접속**: [https://cloud.google.com/free](https://cloud.google.com/free)에 접속하여 로그인 합니다. 
* **프로젝트 생성**: 콘솔 화면 좌상단에 위치한 `프로젝트 선택`에서 `새 프로젝트`를(예: `PoliceAndThief`) 생성합니다.

#### 5.4.2 지도 생성 및 스타일 적용
* **Google Map API Key 생성**: 콘솔 메뉴에서 `Google Maps Platform`으로 이동해 `키 및 사용자 인증정보`탭으로 이동해 상단에서 엑세스 할 서비스`Maps SDK for Android`를 선택한 뒤 `사용자 인증 정보 만들기`를 통해 API Key를 생성합니다.
* **지도 ID 생성**: `지도 관리`탭으로 이동하여 `지도 ID 만들기`에 지도의 이름(예: game_map, setting_map)과 설명을 입력하고 지도 유형을 `Android`로 설정하여 생성 합니다. 이후 언제든 이 탭에서 ID를 확인할 수 있습니다.
* **지도 스타일 생성**: `지도 스타일` 탭으로 이동하여 `스타일 만들기`에서 `JSON`을 눌러 각 지도 스타일에 맞는 JSON을 붙여 넣은뒤 저장과 게시를 누릅니다.
* **지도에 스타일 할당**: 다시 `지도 관리` 탭으로 이동하여 각 지도에 알맞는 스타일을 적용합니다. 

#### 5.4.3 local.properties 작성
```
GOOGLE_MAP_API_KEY={Google Maps Platform API Key}
INGAME_MAP_ID={인게임 맵 스타일이 적용된 지도 ID}
SETTING_MAP_ID={게임 대기방 맵 스타일이 적용된 지도 ID}
```

### 5.5 Naver Reverse Geocode
현재 위치 정보를 대한민국 행정동 이름으로 변환하기위해 **네이버 역 지오코드 서비스**를 사용합니다.

#### 5.5.1 Application 등록
* **Naver Cloud Platform 접속**: [https://www.ncloud.com/v2/?from=ncp_home](https://www.ncloud.com/v2/?from=ncp_home)에 접속하여 로그인 합니다. 
* **Application 등록**: 검색창에 `maps`를 검색해 지도 서비스 페이지로 이동하여 `Application 등록`에 Application이름(예: PoliceAndThief)을 입력하고 `Reverse Geocoding` API서비스를 선택한 뒤 Android 패키지 명(예: com.example.myapp)을 입력한 뒤 등록합니다.

#### 5.5.2 Application Key 확인
* **Client ID와 Secret 확인**: 등록한 앱의 `인증 정보`를 클릭하면 Client ID와 Client Secret을 확인할 수 있습니다.

#### 5.5.3 local.properties 작성
```
CLIENT_ID={Client ID}
...
### 5.6 FCM
채팅방에 새로운 메세지 푸시 알림을 위해 FCM(Firebase Cloud Messaging)알림 서비스를 연동합니다.

#### 5.6.1 프로젝트 생성
* **Firebase 콘솔 접속**: [https://console.firebase.google.com/](https://console.firebase.google.com/)에 접속하여 로그인 합니다.
* **프로젝트 생성**: `프로젝트 추가`에 프로젝트 이름(예: PoliceAndThief)을 입력하여 프로젝트를 생성합니다.

#### 5.6.2 안드로이드 앱 등록
* **Application 등록**: 프로젝트의 대시보드 중앙에서 **안드로이드 아이콘**을 클릭한 뒤 프로젝트 패키지명(예: com.example.myapp)을 입력하여 앱을 등록합니다.

#### 5.6.3 google-services.json 배치
* **google-services.json 다운로드**: `google-services.json 다운로드` 버튼을 클릭하여 파일을 내려받습니다. 
* **google-services.json 배치**: 다운로드 받은 `google-services.json` 파일을 `프로젝트 root/app`에 배치합니다.