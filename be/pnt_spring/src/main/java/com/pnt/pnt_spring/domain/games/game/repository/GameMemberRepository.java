...
public interface GameMemberRepository extends JpaRepository<GameMember, Long> {
    ...
    // 특정 포지션 인원 체크 (POLICE/THIEF 카운트 제한 걸 때)
    long countByGameIdAndGivenPosition(Long gameId, String givenPosition);

    // ready / position update 시 경합 방지
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("select gm from GameMember gm where gm.game.id = :gameId and gm.member.id = :memberId")
